...
public class JZMediaExo extends JZMediaInterface implements Player.Listener {
    ...
    @Override
    public void start() {
        if (isReleased || simpleExoPlayer == null) {
            return;
        }
...
// 5. Create the SimpleExoPlayer instance
            ...
            if (!TextUtils.isEmpty(extUserAgent)) {
                ...
                //userAgent = Util.getUserAgent(context.getApplicationContext(), "StreamHub-" + System.currentTimeMillis());

                userAgent = "Mozilla/5.0 (SMART-TV; Linux; Tizen 6.0) AppleWebKit/537.36 " + "(KHTML, like Gecko) SamsungBrowser/4.0 Chrome/94.0.4606.31 TV Safari/537.36";
            }
    ...
    public void release() {
        ...
        if (tmpMediaPlayer != null) {
            try {
                ...
                tmpMediaPlayer.stop();
                tmpMediaPlayer.clearMediaItems();
